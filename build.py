...
subprocess.run([os.path.join(depot_tools_path,"python.bat"), os.path.join(engine_path,"src","flutter","tools","gn"),"--unoptimized","--target-os",'win',"--windows-cpu","x64"],env=env)
...
subprocess.run([os.path.join(depot_tools_path,"ninja.exe"),"-C",os.path.join(engine_path,"src","out","win_debug_unopt")],env=env)

# subprocess.run([os.path.join(depot_tools_path,"python.bat"), os.path.join(engine_path,"src","flutter","tools","gn"),'--ide=json'],env=env)
# ,'--json-ide-script=%s' % os.path.join(base_path,"gn_to_cmake.py")
